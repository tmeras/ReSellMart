...
export function getProductsByUser(userId: string, page = 0, search = ""): Promise<AxiosResponse<PageResponse<ProductResponse>>> {
    return api.get(`/api/products/users/${ userId }?pageSize=3&pageNumber=${ page }&search=${ search }`);
}

export function getProductsByUserQueryOptions(
    { userId, page, search }: { userId: string, page?: number, search?: string }
) {
    return queryOptions({
        queryKey: (page || page === 0 || search)
            ? ["products", `user ${ userId }`, { page, search }]
